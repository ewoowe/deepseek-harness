...
插件自己的文案提供**七种**语言，全部在 `src/client/locales.ts`：宿主自带的 `zh` 与 `en`，
外加 `ja`、`ko`、`es`、`fr`、`de`。后五种通过**单语言重载**逐个注册——语言包里那份「定义」
才是让一种语言可被选中的东西，本插件只往它上面补自己的字符串，且刻意**不调用 `addLanguage`**。

七本字典都标注为 `Record<MessagesKey, string>`：往 `MessagesKey` 加一个键，七本补齐之前
**编译不过**，所以不会出现「某个键只是忘了翻译、于是静默退回英文」。这个保证比回退链本身更重要。

用词与 `session-messages` 保持一致（它早已带上这五种）——两个插件在同一套界面里，
读者应当两处都看到 `使用量` / `사용량` / `Uso` / `Consommation` / `Verbrauch`。
注册表不带复数规则，所以需要变化的地方一律让**标签在前**：`Turnos: {count}`，
而不是 `{count} turnos`（后者在西班牙语里会读成「1 turnos」）。
...
- **模型显示名依赖 `remote` 层。** 组合里没有它时，表格退回打印原始 id（如 `deepseek-flash`），
  其余行为不变。
- **另外五种语言需要语言包。** `ja` / `ko` / `es` / `fr` / `de` 已由本插件注册，但一种语言要
  **可选**，得由该环境携带的语言包给出定义。没有语言包时这些条目不会出现在选择器里；
  有语言包时这些字符串已经就位。
