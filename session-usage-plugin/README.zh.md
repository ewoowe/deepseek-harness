...
**点一个范围会自动把该范围需要的历史拉进来。** 窗口只装得下最近一段，所以在跑了一个月的会话上点
「7 天」，起初只会看到窗口碰巧覆盖的那部分。插件会逐页往前拉（每页 50 条消息，Session Controller
自己的 pager），**直到窗口最早一轮已早于范围起点就停**——拉到够用为止，而不是把整段历史拖进来回答
一个关于一周的问题。上限 40 页（2000 条）；真超过就由覆盖度行的「载入全部历史」一次到位。

拉进来的历史只多不少：**载入不回缩**，为某个范围加载的部分会一直留着。范围模式下覆盖度行会明确
写出缺口——`· 更早的轮次还没载入`——因为在范围内「已统计 N / N 轮」这句话本身看不出还有更早的。
