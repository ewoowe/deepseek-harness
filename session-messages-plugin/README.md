...
```sh
# 1. Build (re-run after any src change) — both commands run from the repo root
cd session-messages-plugin && npm run build && cd ..

# 2. Install into the profile (back at the repo root)
pnpm dsh plugin --profile web add ./session-messages-plugin
```

`node_modules/.bin/tsx session-messages-plugin/build.mjs` also works from the repo root, without
entering the directory. Note that it relies on the shell shim under `node_modules/.bin/`, which
does not exist on Windows — use `npm run build` above there.

Then **restart** `pnpm dsh web`: a new bundle layer, and any change to an existing one, both need
a restart.
